...
func (socket *SocketTLS) New(name string, port string, tlsCert string, tlsKey string) (chan SocketEvent, error) {
	...
	if err != nil {
		log.Errorf("%s:Port:%s threw an error.\n%v", socket.name, socket.port, err)
		return nil, err
	}
...
func (socket *SocketTLS) run() {
	for {
		...
		if err != nil {
			log.Errorf("%s: New Client connect error.\n%v", socket.name, err)
			socket.eventChan <- SocketEvent{
				Name: "error",
				Data: EventError{
					Error: err,
		...
		go func() {
			...
			log.Debugf("Handshake Done %v, %v", state.HandshakeComplete, state)
			...
			log.Noteln(socket.name + ":New client connected")
...
func (socket *SocketTLS) removeClient(client *ClientTLS) error {
	...
	log.Println("Found client as ", indexToRemove)
