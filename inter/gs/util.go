...
// ProcessCommand turns gs's command string to the
// command struct
func ProcessCommand(msg string) (*Process, error) {
	outCommand := new(Process)
	outCommand.Msg = make(map[string]string)
	data := strings.Split(msg, "\\")

	// TODO:
	// Should maybe return an emtpy Process struct instead
	if len(data) < 1 {
		log.Errorln("Process message invalid")
		return nil, errors.New("Process message invalid")
	}
...
// DecodePassword decodes gs's base64 string used for passwords
...
func BF2Random(randomLen int, source rand.Source) string {
	...
	for i, cache, remain := randomLen-1, source.Int63(), gsLettersIdxMax; i >= 0; {
		if remain == 0 {
