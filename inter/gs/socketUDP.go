...
func (socket *SocketUDP) Close() {
	// Fire closing event
	log.Noteln(socket.name + "Close Port " + socket.port)
...
func (socket *SocketUDP) run() {
	...
	for {
		...
		log.Println("Got UDP Packet:", message)
...
func (socket *SocketUDP) Answer(msgType string, msg map[string]string, msgType2 uint32, addr *net.UDPAddr) error {
	var lena int32
...
func (socket *SocketUDP) Write(message string, addr *net.UDPAddr) {
	...
	if err != nil {
		log.Errorf("%s: Error writing to UDP. Msg:%s Client:%v %v", socket.name, message, addr, err)
		socket.eventChan <- SocketUDPEvent{
			Name: "error",
			Addr: addr,
			Data: err,
